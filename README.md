...
Обязательные колонки из задания сохранены. Дополнительные признаки в CSV позволяют проверить, из чего складывается вывод. Полные схемы и формулы приведены в [документации бэкенда](backend/README.md).
...
| Ошибка авторизации или модели | Вход в Claude Code или `ANTHROPIC_API_KEY`, доступ аккаунта к `AGENT_MODEL`, путь `CLAUDE_CLI_PATH`; подробности в [документации бэкенда](backend/README.md#авторизация-и-версия-cli) |
...
В коде уже есть переход к выборочной betweenness после 20 тысяч узлов, но это не подтверждает готовность всей системы к миллиону. Для такого масштаба также нужны загрузка графа в интерфейс по частям, пересмотр нормализации приоритетов и замеры памяти и времени. План изменений подробнее изложен в [технической документации](backend/README.md#что-изменится-при-масштабе-1-млн-узлов).
