...
export function wrapContentWithHeader(text, contentHeader) {
    return contentHeader ? `${contentHeader}\n${text}` : wrapContent(text);
}
export function wrapContext(json) {
    return `\n${CONTEXT_MARKER}\n${json}`;
}
// Separate inv-lang code from inline context in a message, stripping
// display-only noise: the bare end marker and partial streaming tails. The
// raw message bytes in the store stay untouched.
export function separateContentAndContext(raw) {
 ...
 * unfenced.
 */
export function hasLangSyntax(content) {
...
// Strip every `]]>inv:end` marker LINE and report presence. Mirrors the
...
// ---------------------------------------------------------------------------
// Artifact sentinel — the `]]>inv:artifact` member of the inline-sentinel
...
// Carrier: `]]>inv:artifact <header-json>\n<program>` (header-only on a
...
// ---------------------------------------------------------------------------
const ARTIFACT_MARKER = `${INV_INLINE_SENTINEL}artifact`;
const ARTIFACT_KIND_BY_TYPE = {
    slides: "slides",
    presentation: "slides",
    report: "report",
};
 ...
 */
export function parseArtifactSentinel(raw) {
    ...
    if (typeof parsed !== "object" || parsed === null)
        return null;
    const h = parsed;
    if (typeof h["artifact_id"] !== "string" || h["artifact_id"] === "")
        return null;
...
//# sourceMappingURL=sentinelParser.js.map