...
import { defineComponent, parseStructuredRules, reactive, useFormValidation, useIsStreaming, useStateField, } from "@inv/lang";
import React from "react";
import { z } from "zod/v4";
import { RadioGroup as InvRadioGroup } from "../../components/RadioGroup";
import { RadioItem as InvRadioItem } from "../../components/RadioItem";
...
export const RadioGroup = defineComponent({
    ...
    component: ({ props }) => {
        ...
        if (!items.length)
            return null;
        return (_jsx(InvRadioGroup, { name: field.name, value: value ?? "", onValueChange: (val) => {
                field.setValue(val);
                if (hasRules) {
                    formValidation?.validateField(field.name, val, rules);
                }
            }, disabled: isStreaming, children: items.map((item, i) => (_jsx(InvRadioItem, { value: item.props.value, label: item.props.label, description: item.props.description || "" }, i))) }));
    },
});
