...
import { defineComponent, useGetFieldValue, useIsStreaming, useSetFieldValue, useTriggerAction, } from "@inv/lang";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { EditableTableChangesBar, EditableTable as InvEditableTable, } from "../../components/EditableTable";
import { EditableTableSchema } from "./schema";
export * from "./schema";
function EditableTableRenderer({ props }) {
    ...
    const toRows = useCallback((data) => data.map((row) => {
        const valuesMap = columnKeys.reduce((acc, key, index) => {
            acc[key] = row.values[index];
            return acc;
        }, {});
        return { ...valuesMap, id: row.id };
    }), [columnKeys]);
    const fromRows = useCallback((rows) => rows.map((row) => ({ id: row.id, values: columnKeys.map((key) => row[key]) })), [columnKeys]);
    const existingValue = getFieldValue(tableName, tableName);
    const baselineDataMapRef = useRef(undefined);
    ...
    useEffect(() => {
        if (!isStreaming && baselineDataMapRef.current === undefined) {
            baselineDataMapRef.current = new Map(toRows(existingValue || initialData).map((row) => [row.id, row]));
        }
    ...
    const calculateChangedCellCount = useCallback((changedData, baseline) => {
        ...
        baseline.forEach((baseValues, id) => {
            ...
            for (const key of columnKeys) {
                if (changedValues[key] !== baseValues[key])
                    count++;
            }
        });
    ...
    return (_jsxs(_Fragment, { children: [_jsx(InvEditableTable, { data: tableData, columns: columns, onDataChange: handleDataChange }), _jsx(EditableTableChangesBar, { changedCellCount: changedCellCount, onReset: handleReset, onSave: handleSubmit })] }));
}
