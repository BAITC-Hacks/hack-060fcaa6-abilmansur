...
function lastContent(scroller) {
    const messages = scroller.querySelector(".inv-agent-thread-messages");
    const last = messages?.lastElementChild;
    if (!last)
        return null;
    const children = Array.from(last.children);
    if (children.length === 0)
        return { el: last, bottom: last.getBoundingClientRect().bottom };
    let el = children[0];
...
export const ScrollToLatest = ({ scrollRef, }) => {
    ...
    if (!belowFold)
        return null;
    return (_jsx("button", { type: "button", className: "inv-agent-thread-scroll-latest", onClick: jump, "aria-label": "Scroll to latest message", children: _jsxs("svg", { className: "inv-agent-thread-scroll-latest__arrow", width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": true, children: [_jsx("path", { d: "M12 5v14" }), _jsx("path", { d: "m19 12-7 7-7-7" })] }) }));
};
//# sourceMappingURL=ScrollToLatest.js.map