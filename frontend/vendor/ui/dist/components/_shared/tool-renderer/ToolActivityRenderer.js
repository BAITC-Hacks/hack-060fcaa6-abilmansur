...
import { artifactViewId, parseArtifactViewId, useDetailedView, useDetailedViewStore, useThreadContextStore, } from "@inv/headless";
...
function runRenderer(renderer, activity) {
    return renderer.parser({ args: activity.toolCall.function.arguments, response: activity.result ?? null }, { isStreaming: activity.status === "streaming" || activity.status === "executing" });
}
/**
 ...
 * @internal
 */
export function ToolActivityRenderer({ renderer, activity, detailedViewPanel: DetailedViewPanel = DefaultDetailedViewPanel, fallback = null, }) {
    ...
    const { parsed, error } = useMemo(() => {
        try {
            return { parsed: runRenderer(renderer, activity), error: null };
        }
        catch (e) {
    ...
    useEffect(() => {
        ...
        if (dv.activeDetailedViewId === prev)
    ...
    useEffect(() => {
        ...
        if (!active || active === viewId)
            return;
        const parsed = parseArtifactViewId(active);
        if (!parsed || parsed.id !== meta.id)
            return;
    ...
    if (error) {
        return _jsx(ToolCallErrorFallback, { error: error, toolName: activity.toolName });
    }
    ...
    if (activity.isError || parsed === null)
    ...
    return (_jsxs(_Fragment, { children: [renderer.preview(parsed.props, controls), _jsx(DetailedViewPanel, { viewId: viewId, title: meta?.heading ?? "Detailed view", children: renderer.actual(parsed.props, controls) })] }));
}
//# sourceMappingURL=ToolActivityRenderer.js.map