...
const ChartContainer = forwardRef(({ id, className, children, config, rechartsProps, style, ...props }, ref) => {
    ...
    return (_jsx(ChartContext.Provider, { value: { config, id: chartId }, children: _jsxs("div", { "data-chart": chartId, ref: ref, className: clsx("inv-chart-container", className), style: {
...
function ChartTooltipContentRender({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey, showPercentage = false, }, ref) {
    ...
    const tooltipLabel = useMemo(() => {
        ...
        if (labelFormatter) {
            return (_jsx("div", { className: clsx("inv-chart-tooltip-label-heavy", labelClassName), children: labelFormatter(value, payload) }));
        }
        if (!value) {
            return null;
        }
        return _jsx("div", { className: clsx("inv-chart-tooltip-label", labelClassName), children: value });
    }, [label, labelFormatter, payload, hideLabel, labelClassName, config, labelKey]);
    if (!active || !payload?.length) {
        return null;
    }
    const nestLabel = payload.length === 1 && indicator !== "dot";
    return (_jsxs("div", { ref: ref, className: clsx("inv-chart-tooltip", className), children: [!nestLabel && tooltipLabel, _jsx("div", { className: "inv-chart-tooltip-content", children: payload.map((item, index) => {
                    const key = `${nameKey ?? item.name ?? item.dataKey ?? "value"}`;
                    const itemConfig = getPayloadConfigFromPayload(config, item, key);
                    const indicatorColor = (color ?? item.payload.fill) || item.color;
                    return (_jsx("div", { className: clsx("inv-chart-tooltip-content-item", indicator === "dot" && "inv-chart-tooltip-content-item--dot"), children: formatter && item?.value !== undefined && item.name ? (formatter(item.value, item.name, item, index, item.payload)) : (_jsxs(_Fragment, { children: [itemConfig?.icon ? (_jsx(itemConfig.icon, {})) : (!hideIndicator && (_jsx("div", { className: clsx("inv-chart-tooltip-content-indicator", `inv-chart-tooltip-content-indicator--${indicator}`), style: {
                                        "--color-bg": indicatorColor,
                                        "--color-border": indicatorColor,
                                    } }))), _jsxs("div", { className: clsx("inv-chart-tooltip-content-value-wrapper", nestLabel
                                        ? "inv-chart-tooltip-content-value-wrapper--nested"
                                        : "inv-chart-tooltip-content-value-wrapper--standard"), children: [_jsxs("div", { className: "inv-chart-tooltip-content-label", children: [nestLabel && tooltipLabel, _jsx("span", { children: itemConfig?.label || item.name })] }), item.value !== undefined && (_jsxs("span", { className: clsx("inv-chart-tooltip-content-value", showPercentage && "percentage"), children: [item.value.toLocaleString(), showPercentage ? "%" : ""] }))] })] })) }, item.dataKey));
                }) })] }));
}
...
const ChartLegendContent = forwardRef(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
    ...
    return (_jsx("div", { ref: ref, className: clsx("inv-chart-legend", `inv-chart-legend--${verticalAlign}`, className), children: payloadWithKeys?.map((item) => {
            const key = `${nameKey || item.dataKey || "value"}`;
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            return (_jsxs("div", { className: "inv-chart-legend-item", children: [itemConfig?.icon && !hideIcon ? (_jsx(itemConfig.icon, {})) : (_jsx("div", { className: "inv-chart-legend-item-indicator", style: { backgroundColor: item.color } })), _jsx("span", { className: "inv-chart-legend-item-label", children: itemConfig?.label })] }, item.uniqueKey));
        }) }));
});
