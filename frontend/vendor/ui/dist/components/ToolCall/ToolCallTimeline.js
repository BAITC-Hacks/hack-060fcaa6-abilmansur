...
const TimelineStepRow = ({ step, isLast, detailedViewPanel, forceDefault, }) => {
    if (step.type === "text") {
        return (_jsx("div", { className: "inv-tool-call-timeline__text-step", children: _jsx(MarkDownRenderer, { textMarkdown: step.text, className: "inv-tool-call-timeline__text-step-body" }) }));
    }
    return (_jsx(TimelineEntry, { activity: step.activity, isLast: isLast, detailedViewPanel: detailedViewPanel, forceDefault: forceDefault }));
};
const REVEAL_INTERVAL = 600;
const NO_FADE = { top: false, bottom: false };
 ...
 * @category Components
 */
export function ToolCallTimeline({ activities, steps, isLast = false, detailedViewPanel, forceDefault = false, awaitingResponse = false, }) {
    const displaySteps = steps ?? activities.map((activity) => ({ type: "activity", activity }));
    ...
    const thinking = isThreadRunning &&
        isLast &&
        activities.length > 0 &&
    ...
    const handleItemsScroll = useCallback(() => {
        const el = itemsRef.current;
        if (el && distanceFromBottom(el) < 24)
            followRef.current = true;
        measureEdges();
    }, [measureEdges]);
    ...
    useLayoutEffect(() => {
        if (!thinking || !followRef.current)
            return;
        const el = itemsRef.current;
        if (!el || distanceFromBottom(el) < 1)
            return;
    ...
    const currentReady = !revealingStep || revealingStep.type === "text"
        ? true
    ...
    if (activities.length === 0 || displaySteps.length === 0)
        return null;
    ...
    const liveLabel = current.type === "text"
        ? current.text
    ...
    const toggleLabel = working
        ...
        : failedCount > 0
            ...
            : "Behind the scenes";
    return (_jsxs("div", { className: "inv-behind-the-scenes", children: [_jsx("div", { role: "status", "aria-live": "polite", style: VISUALLY_HIDDEN, children: liveLabel }), _jsxs("button", { className: "inv-behind-the-scenes__toggle", type: "button", "aria-expanded": isOpen, onClick: () => {
                ...
                }, children: [_jsx("span", { className: clsx("inv-behind-the-scenes__toggle-label", {
                            // While the run is live the label itself shimmers, matching the
                            // running tool rows below it.
                            "inv-behind-the-scenes__toggle-label--shimmer": working,
                        }), children: toggleLabel }), isOpen ? (_jsx(ChevronUp, { size: 14, className: "inv-behind-the-scenes__toggle-icon" })) : (_jsx(ChevronDown, { size: 14, className: "inv-behind-the-scenes__toggle-icon" }))] }), isLast && !expanded && (_jsx("div", { className: clsx("inv-behind-the-scenes__compact", {
                    "inv-behind-the-scenes__compact--closed": !showCompact,
                }), inert: !showCompact, children: _jsx("div", { className: "inv-behind-the-scenes__compact-inner", children: _jsx("div", { className: itemsClassName, ref: attachItems, onScroll: handleItemsScroll, onWheel: handleItemsWheel, onTouchMove: handleItemsTouchMove, children: displaySteps.slice(0, revealedCount).map((step, idx) => (_jsx("div", { className: "inv-behind-the-scenes__reveal-item", style: { width: "100%" }, children: _jsx(TimelineStepRow, { step: step, isLast: idx === revealedCount - 1, detailedViewPanel: detailedViewPanel, forceDefault: forceDefault }) }, stepKey(step)))) }) }) })), expanded && (_jsx("div", { className: itemsClassName, ref: attachItems, onScroll: handleItemsScroll, onWheel: handleItemsWheel, onTouchMove: handleItemsTouchMove, children: displaySteps.map((step, idx) => (_jsx("div", { style: { width: "100%" }, children: _jsx(TimelineStepRow, { step: step, isLast: isLast && idx === displaySteps.length - 1, detailedViewPanel: detailedViewPanel, forceDefault: forceDefault }) }, stepKey(step)))) }))] }));
}
//# sourceMappingURL=ToolCallTimeline.js.map